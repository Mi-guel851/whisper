...
export default function PublicFeedPage() {
  ...
  return (
    <main className="min-h-screen theme-bg-gradient px-4 pb-28 pt-10 text-white"><div className="mx-auto max-w-xl"><BackButton /><div className="mb-7 mt-5"><h1 className="text-4xl font-black">Public Feed</h1><p className="mt-1 text-sm text-gray-400">Real thoughts from the Whisper community.</p></div>
      <GlassPanel strong className="mb-7 rounded-3xl p-5"><form onSubmit={createPost}><textarea value={body} onChange={(event) => setBody(event.target.value)} maxLength={500} rows={3} placeholder="Share a thought with the Whisper community..." className="w-full resize-none bg-transparent text-sm text-white outline-none placeholder:text-gray-500" /><button type="button" onClick={() => setBody(SUGGESTED_POST)} className="mt-3 w-full rounded-2xl border border-cyan-300/15 bg-cyan-300/5 px-3 py-2 text-left text-xs text-cyan-100 transition hover:bg-cyan-300/10"><span className="font-bold text-cyan-300">Suggestion:</span> {SUGGESTED_POST}</button><div className="mt-3 flex items-center justify-between gap-3 border-t border-white/10 pt-3"><div className="min-w-0 text-xs text-gray-400"><span className="block">Your Whisper link will be attached automatically.</span>{ownLink && <Link href={ownLink} className="truncate text-cyan-300">whisper.app{ownLink}</Link>}</div><button type="submit" disabled={!cleanBody || posting || !ownLink} className="flex shrink-0 items-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-400 to-purple-500 px-4 py-2.5 text-sm font-black text-black disabled:opacity-50"><Send size={15} />{posting ? "Posting" : "Post"}</button></div></form></GlassPanel>
