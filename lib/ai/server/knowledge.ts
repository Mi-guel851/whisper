...
Find a Match: below the Friends tabs, tap the Find a Match panel and Scan. A short radar sweep runs, then a ranked list of people shows up — closest first is people in your own self-declared country (the one you chose at sign-up; Whisper never uses device geolocation), then who has been active recently. Friends, pending requests, blocked users and people who opted out never appear. You add them with the usual Add Friend flow, and "Scan again" shows the next page. Opt out any time in Settings → "Appear in Find a Match" (on by default).`,
  ...
  feed: {
    ...
    text: `Public Feed: Discover → Public Feed. Write up to 500 characters and post — posting costs 2 coins, and your Whisper link is attached automatically so readers can send you anonymous whispers. Posts clear after 24 hours. The "AI Write" button generates a fresh post idea each time you tap it from a topic-balanced bank of more than 100 public-feed ideas. You can like posts, reply for free, share a post, and delete your own. New posts, likes and replies appear live, and the Discover tile shows a badge for unread feed activity.`,
  },
