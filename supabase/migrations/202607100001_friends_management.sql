-- Discover People friends system
...
do $$ begin alter publication supabase_realtime add table public.friends; exception when duplicate_object then null; end $$;
do $$ begin alter publication supabase_realtime add table public.coins; exception when duplicate_object then null; end $$;
do $$ begin alter publication supabase_realtime add table public.coin_transactions; exception when duplicate_object then null; end $$;
...
drop policy if exists "Receivers can delete pending friend requests" on public.friend_requests;
create policy "Receivers can delete pending friend requests" on public.friend_requests
  for delete using (auth.uid() = receiver_id and status = 'pending');
...
drop function if exists public.connect_with_foreigner(uuid);
...
begin
  ...
  if exists (select 1 from public.friends where user_id = request_row.receiver_id and friend_id = request_row.sender_id) then
    raise exception 'Already friends';
  end if;
  ...
  if current_balance < 20 then raise exception 'Not enough coins.'; end if;
  ...
  values (auth.uid(), 'spend', -20, 'Accept Friend Request', jsonb_build_object('request_id', request_id, 'sender_id', request_row.sender_id));
  ...
  values (request_row.receiver_id, conversation_id)
