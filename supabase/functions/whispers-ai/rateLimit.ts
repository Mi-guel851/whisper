 ...
 *
 *  1. In-memory (this isolate). Free, instant, and it also catches the case a
 *     database can't cheaply catch — the same user firing a second question
 *     while the first is still in flight. It resets when the isolate recycles
 *     and isn't shared between concurrent instances, so on its own it's a speed
 ...
 *     actually protects the Hugging Face allowance.
 *
 * Layer 2 is optional: if its migration hasn't been applied the function keeps
...
/** Users with a request currently being served by this isolate. */
 ...
 * the user stays locked out of their own assistant until the isolate recycles.
 */
...
/**
 * Flipped off permanently for this isolate the first time the RPC turns out not
  ...
  try {
    ...
    if (error) {
      if (looksMissing(error)) {
        ...
        console.warn(
          `[whispers-ai] ${RPC} not found — durable rate limiting is off until the migration is applied. In-memory limits still apply.`
        );
        return permissive;
      }
