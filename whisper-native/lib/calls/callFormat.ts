/**
 * Call formatting — a verbatim port of the web app's `lib/calls/callFormat.ts`.
 *
 * Pure on purpose: the in-call sheet re-renders every second, and "3:07" must
 * be the same string on every render and on every device — a formatter that
 * drifts between renders reads as a broken timer, which is how the user learns
 * not to trust the rest of the call UI.
 */
...
export function formatCallDuration(ms: number): string {
  ...
  /* Hours only past the hour — a "1:02:03" on a one-hour call would look like
     a formatting error more than it would look impressive. */
  if (minutes >= 60) {
...
/**
 * Call-log entry rendering for the chat timeline.
 *
 ...
 * end_call_log / the expiry sweep wrote.
 */
...
export type CallEntryInfo = {
  ...
  callee_id: string;
  started_at: string;
  /** When someone picked up. Absent on rows written before 202609120003. */
...
export function describeCallEntry(
  ...
  const stale =
    (entry.status === "ringing" || entry.status === "answered") &&
