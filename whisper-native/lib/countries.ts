 ...
 * The country list.
 *
 * A direct copy of the web app's `lib/countries.ts` — same `Country` shape,
 * same ISO codes, same dial codes — because complete-profile writes `country` /
 * `country_code` / `dial_code` and the web's CountryPhoneInput writes the very
 * same values for the very same people. One list, two clients, no drift.
 */
